...
- не выбирает единственный DOM-узел напрямую: вместо этого строит упорядоченную цепочку кандидатов вокруг элемента под курсором, стартует с рекомендованного уровня и позволяет перейти к более мелкому или более крупному контейнеру перед отправкой;
- показывает overlay с действиями **«Мельче» / «Крупнее» / «Отправить» / «Отмена»**;
...
UI расширения использует светлую оливковую тему:

- popup следует общей светлой оливковой палитре;
- overlay DOM picker и toast используют тот же визуальный стиль, чтобы сценарий выбора выглядел единообразно.
