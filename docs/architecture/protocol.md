...
| `client.setTargetModel` | browser → broker | Запросить смену модели выбранной Pi-цели |
| `client.modelSetResult` | broker → browser | Вернуть результат смены модели |
| `client.runtimeState` | broker → browser | Передать состояние модели/контекста подписанной Pi-цели |
| `client.availableModels` | broker → browser | Передать список моделей подписанной Pi-цели |
...
| `target.runtimeState` | Pi → broker | Опубликовать текущую модель, заполненность контекста и idle-state |
| `target.availableModels` | Pi → broker | Опубликовать доступные модели Pi-сессии |
| `target.setModel` | broker → Pi | Передать команде target запрос смены модели |
| `target.modelSetResult` | Pi → broker | Вернуть результат смены модели |
...
| `assistant.model.set` | Сменить модель выбранной Pi-сессии (`provider`, `modelId`) |
...
7. Публиковать `target.runtimeState` и `target.availableModels` после подключения и при изменении модели/контекста.
8. При получении `target.setModel` найти модель в `ctx.modelRegistry.getAvailable()`, вызвать `pi.setModel(model)` и ответить `target.modelSetResult`.
9. По штатному завершению отправить `target.unregister`.
