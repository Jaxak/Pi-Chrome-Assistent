...
export type SidePanelBrokerConnectionState = {
  online: boolean;
  statusText: string;
};

export type SidePanelBrokerSocketEventName = "open" | "message" | "error" | "close";
export type SidePanelBrokerSocketEventListener = (event?: { data?: string }) => void;

export interface SidePanelBrokerSocket {
  readyState: number;
  send(data: string): void;
  close(): void;
  addEventListener(
    eventName: SidePanelBrokerSocketEventName,
    listener: SidePanelBrokerSocketEventListener,
    options?: { once?: boolean },
  ): void;
  removeEventListener(eventName: SidePanelBrokerSocketEventName, listener: SidePanelBrokerSocketEventListener): void;
}

export type SidePanelBrokerClientOptions = {
  browserToken: string;
  selectedTargetId?: string;
  brokerUrl?: string;
  webSocketFactory?: (url: string) => SidePanelBrokerSocket;
...
function isOpen(socket: SidePanelBrokerSocket | undefined): socket is SidePanelBrokerSocket {
  return socket?.readyState === SOCKET_OPEN;
}

function sendEnvelope(
  socket: SidePanelBrokerSocket,
  envelope: {
    type: string;
    requestId?: string;
    payload?: unknown;
...
export class SidePanelBrokerClient {
  ...
  private socket: SidePanelBrokerSocket | undefined;
